...
class PygView(object):
    ...
    def loadresources(self):
        ...
        for x in range(0,self.width, self.grid):
            pygame.draw.line(self.background, (128,0,0), (x,0), (x, self.height))
        for y in range(0, self.height, self.grid):
            pygame.draw.line(self.background, (128,0,0), (0,y), (self.width, y))
    ...
    def run(self):
        ...
        while running:
            ...
            write(self.screen, "FPS: {:6.3}  PLAYTIME: {:6.3} SECONDS".format(
                           self.clock.get_fps(), self.playtime), y=50)
            #write(self.screen, "player1: {}".format(self.tux1.nextmove), y=70) 
            # ----- turn indicator
            write(self.screen, "next turn in {:6.3} seconds".format(self.turn_duration - self.turntime), y=20)
            ...
            for b in self.bulletgroup:
                  ...
                  for pos in b.trail:
                     # pygame.draw.line(surface, color, (startx, starty), (endx, endy), width=1)
                     pygame.draw.line(self.screen, (color,0,0), (oldx, oldy), (pos[0],pos[1]), color // 100 +5)
                     oldx = pos[0]
                     oldy = pos[1]
                     color-=1
            ...
            write(self.screen, "Press w,a,s,d and R,L,U,D to steer tux and tux2", x=self.width//2, y=660, center=True, color=(200,0,0))
            write(self.screen, "Press space and m to fire from tux and tux2", x=self.width//2, y=690, center=True, color=(200,0,0))
            # --------- next frame ---------------
            pygame.display.flip()
        pygame.quit()
